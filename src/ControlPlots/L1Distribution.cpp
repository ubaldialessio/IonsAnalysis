...
std::vector<TString> histogram = {"qL1_den","qL1_den_ltof","qL1_den_ltof_l9_05","qL1_den_ltof_l9_1"};
std::map<TString,TString> name = { {"qL1_den","Den"},
                                   {"qL1_den_ltof","Den+LTof"},
                                   {"qL1_den_ltof_l9_05","Den+LTof+UnbL9"},
                                   {"qL1_den_ltof_l9_1","Den+LTof+UnbL9>"} };
...
int main(int argc, char **argv) {
    ...
    if (mode=="dat" || mode=="both") {
        dataFiles = getDistributionFiles(argc,argv,charge_number_dat,"data","L1");
        dataHist = getHistogram<TH1D*>(dataFiles,histogram);
    }
    if (mode=="mc" || mode=="both") {
        mcFiles = getDistributionFiles(argc,argv,charge_number_mc,"mc","L1");
        mcHist  = getHistogram<TH1D*>(mcFiles,histogram);
    }

    if (mode == "dat") {
        printDistributions(dataHist, {}, charge_number_dat, "l1", histogram, name);
    } else if (mode == "mc") {
        printDistributions({}, mcHist, charge_number_mc, "l1", histogram, name);
    } else if (mode == "both") {
        printDistributions(dataHist, mcHist, charge_number_dat, "l1", histogram, name);
    } else {
