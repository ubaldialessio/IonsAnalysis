...
std::vector<TString> fluxName = {"unf_factor"};
...
void print(std::vector<TH1D*> fluxes, std::vector<unsigned int> chNum, TString timePeriod) {
    ...
    for (int j=0; j<nHistPerFile; j++) {
        ...
        for (int i=0; i<chNum.size(); i++) {
            ...
            if (i==0) setTitle(fluxes[j+nHistPerFile*i], "", "R (GV)", "Unfolding factor", chNum[i]);	
			...
			adjustZoomX(fluxes[j+nHistPerFile*i],"unfFact");
			adjustZoomY(fluxes[j+nHistPerFile*i],"unfFact",chNum[i]);
			formatMarkerSize(fluxes[j+nHistPerFile*i],1);
			fluxes[j+nHistPerFile*i]->SetMarkerStyle(20+i);
			...
			a->AddEntry(fluxes[j+nHistPerFile*i],Form("%s",getIonName(chNum[i]).Data()));
			a->SetTextSize(0.031);
			gPad->SetGridy();
            gPad->SetLogx();
        }
