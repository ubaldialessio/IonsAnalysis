...
std::pair<std::vector<T>, std::vector<T>> getEffHist(std::vector<TFile *> v, TString opt) {
    ...
    for (const auto &file : v) {
        for (const auto & name : eff) {
			...
			if (hist) {
				if ( (opt == "dat" || opt == "both") && std::find(da_eff.begin(),da_eff.end(), name) != da_eff.end() ) {
					printf("Retrieving histogram : %s \n",name.Data());
					effHist.first.push_back(hist);
				} else if ( (opt == "mc" || opt == "both") && std::find(mc_eff.begin(),mc_eff.end(), name) != mc_eff.end() ) {
					printf("Retrieving histogram : %s \n",name.Data());
